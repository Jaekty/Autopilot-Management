...
<#
 ...
 1.1.2 - 07.05.2024 - Replaced deprecated Enterprise App "Microsoft Intune PowerShell" with "Microsoft Graph Command Line Tools" for authentication. 
                      Bugfix: The last 20 objects during group tag change were ignored.
 ...
 1.3.0 - 25.01.2025 - Rewrote some code, primarily deletion; added delete options only Intune, only Autopilot or both. 
                      Bugfix Autopilot upload, was missing "Assigned User" and Group Tag handling was off.
 1.3.1 - 07.02.2025 - Bugfix Autopilot upload.
 1.3.2 - 03.07.2025 - Updating group tag made into function. Renamed functions to avoid conflict. Rename button backup to export. 
                      Bugfix: Support for encoding UTF8 characters; visually and in graph queries. Last batch would not be processed if exactly 20 objects. 
 1.3.3 - 13.02.2026 - PowerShell 7.x support. Improved Autopilot hash import with error handling
 1.4.0 - 22.07.2026 - Improvements: Update group tag and delete optimization, running within its own thread outside main UI. Some code cleaup. 
                      Selection count. Throttle feedback in progressbar. Label showing current action running. 
                      Bugfixes: throttle was not detected during update and possibly deletion. Requery after sorting incorrectly applied Intune and Autopilot profile info to datagrid
 1.4.1 - 29.08.2026 - Bugfix: Selection count check for update group tag count was not readable in runspace, moved to UI thread.
#>
############################
###   BUILD GUI   ##########
############################
...
$inputXaml = @"
<Window x:Class="AutopilotManagement.MainWindow FontFamily="Segoe UI"
    ...
    <Grid>
        ...
        <Label x:Name="lblAction" Content="Action: Idle" HorizontalAlignment="Left" Margin="245,40,0,0" VerticalAlignment="Top" FontSize="12" Visibility="Visible"/>
        ...
        <Button x:Name="btnExportSelection" Content="Export" HorizontalAlignment="Left" Margin="410,50,0,0" VerticalAlignment="Top" Height="28" Width="76" FontSize="16" IsEnabled="False" RenderTransformOrigin="0.461,0.037"/>
...
Function Get-AMCodeFlowAuthToken {
...
Function Start-AMPrepareEnvironment {
    #Set session to use TLS 1.2
    [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
    
    #Identify PS version
    if ([Version]$PSVersionTable.PSVersion -gt [Version]"7.0") { $core = $true; Write-Host "Version PS 7+ detected." } else { $core = $false; "Version PS 5.1 detected." }
    
    #syncHash variables
    $syncHash.workDirRoot = "$($env:LOCALAPPDATA)\AutopilotManagement"
    $syncHash.workDirTemp = "$($syncHash.workDirRoot)\Temp"
    if ($core) { $syncHash.workDirDll = "$($syncHash.workDirRoot)\dll\dotnetcore3" } else { $syncHash.workDirDll = "$($syncHash.workDirRoot)\dll\dotnet46" }

    #GitHub information
    #$gitTag = "v1.0.864.35"
    $gitTag = "v1.0.3650.0"
    if ($core) {
        $gitDlls = "https://github.com/Jaekty/WebView2/raw/refs/tags/$gitTag/dotnetcore3"
    } else {
        $gitDlls = "https://github.com/Jaekty/WebView2/raw/refs/tags/$gitTag/dotnet46"
    }
    ...
    try {
        ...
        if ($downloadDlls) {
            Invoke-RestMethod -Uri "$gitDlls/Microsoft.Web.WebView2.Core.dll" -OutFile "$($syncHash.workDirDll)\Microsoft.Web.WebView2.Core.dll" -ErrorAction Stop
            Invoke-RestMethod -Uri "$gitDlls/Microsoft.Web.WebView2.WinForms.dll" -OutFile "$($syncHash.workDirDll)\Microsoft.Web.WebView2.WinForms.dll" -ErrorAction Stop
            Invoke-RestMethod -Uri "$gitDlls/WebView2Loader.dll" -OutFile "$($syncHash.workDirDll)\WebView2Loader.dll" -ErrorAction Stop
        }
    } catch {
    ...
    try {
        ...
        Write-Host "Autopilot Management version check failed."
    }
}

Function Start-AMCleanupEnvironment {
    ...
    #Cleanup after exit
    Remove-AMRunspace -Custom "RefreshToken" -Force #Force stop refreshing token
    Remove-AMRunspace -Custom "ImportAPDevices" #Don't force, let it finish
    Remove-AMRunspace -Custom "WatchAPImportRunspace" #Don't force, let it finish
    Remove-AMRunspace -Custom "WatchUpdateGroupTagRunspace" -Force #Force stop tracing group tag update runspace
    Remove-AMRunspace -Custom "WatchQueryAutopilotDevicesRunspace" -Force #Force stop tracing Autopilot device query runspace
}

Function Disconnect-AMAzure {
...
Function Confirm-AMTokenValidity {
    Param(
    [Parameter(
        Mandatory=$true,
    ...
    do {
        ...
        if (($syncHash.tokenAcquired).AddSeconds($syncHash.token.ext_expires_in - $refreshTimer) -gt (Get-Date)) {
            ...
            try {
                #Refresh token
                $ErrorActionPreference = "Stop"
                $tempToken = Get-AMCodeFlowAuthToken -CurrentToken $syncHash.token -ClientId $syncHash.clientId
...
Function Disable-AMUI {
    ...
    $syncHash.var_btnExportSelection.IsEnabled = $false
    ...
    Update-AMProgressbar -ProgressbarObject $syncHash.var_progressBar -LabelObject $syncHash.var_lblAction -Action "Idle" -Percent 0
    Update-AMCache -Object $syncHash.var_lblCacheSize
    Remove-AMRunspace -Custom "RefreshToken" -Force #Force stop refreshing token
    Write-Host "UI access disabled."
    return
}

Function Update-AMProgressbar {
    [CmdletBinding(DefaultParameterSetName = "Percent")]
    Param(
        [Parameter(
            Mandatory=$true,
            HelpMessage="WPF object of progressbar to update.")]
            $ProgressbarObject,
        [Parameter(
            Mandatory=$true,
            HelpMessage="WPF object of label to update.")]
            $LabelObject,
        [Parameter(
            Mandatory=$true,
            HelpMessage="Action being performed.")]
            $Action,
        [Parameter(
            ...
            [int]$Percent,
        [Parameter(
            ParameterSetName = "Throttled",
            Mandatory=$true,
            HelpMessage="Show progress bar text when throttled.")]
            [bool]$Throttled
    )
    
    # Update label with action performing
    $LabelObject.Dispatcher.Invoke([action] { 
        $LabelObject.Content = "Action: $($Action)"
    })

    # Set percent value on progress bar
    if ($PSCmdlet.ParameterSetName -eq "Percent") {
        $ProgressbarObject.Dispatcher.Invoke([action] { 
            $ProgressbarObject.value = "$($Percent)" 
        })
    }
    # Show throttle status on progress bar
    if ($PSCmdlet.ParameterSetName -eq "Throttled") {
        if ($Throttled) {
            $LabelObject.Dispatcher.Invoke([action] { 
                $LabelObject.Content = "Action: $($Action) | Throttled"
            })
            $ProgressbarObject.Dispatcher.Invoke([action]{ 
                $ProgressbarObject.Foreground = [System.Windows.Media.Brushes]::Orange 
            })
        } else {
            $LabelObject.Dispatcher.Invoke([action] { 
                $LabelObject.Content = "Action: $($Action)"
            ...
            $ProgressbarObject.Dispatcher.Invoke([action]{ 
                $ProgressbarObject.Foreground = [System.Windows.Media.Brushes]::Green 
            })
        }
    }
}
...
Function Update-AMGridStatsLabel {
    if ($null -eq $syncHash.selectedCount) {
        $syncHash.selectedCount = 0
    }
    $cacheCount = 0
    if ($syncHash.cache) {
        $cacheCount = $syncHash.cache.Count
    }
    $gridCount = $syncHash.var_datagridResults.Items.Count
    Update-AMCache -Object $syncHash.var_lblCacheSize -Text "Cache: $cacheCount devices | Grid: $gridCount devices | Selected: $($syncHash.selectedCount) devices" -Enabled
}

Function Import-AMCsvDevices {
...
Function Search-AMAutopilotDevice {
    #Check token status
    if ((Confirm-AMTokenValidity -TokenAcquired $syncHash.tokenAcquired -TokenLifetime $syncHash.token.expires_in) -ne $true) {
    ...
    if ($useCache -eq $false) {
        ...
        if ($result) {
            ...
            $syncHash.var_btnExportSelection.IsEnabled = $true
            $syncHash.var_chkboxCache.IsEnabled = $true
            #Auto-enable cache query
            $syncHash.var_chkboxCache.IsChecked = $true
            Update-AMCache -Object $syncHash.var_lblCacheSize -Text "Cache size: $($result.Count) devices" -Enabled
        } else {
            if ($serialnumber) {
        ...
        if ($intune) {
            $datagridLookup = @{}
            foreach ($item in $syncHash.var_datagridResults.Items) {
                $datagridLookup[$item.id] = $item
            }
            $objectsFinalBatch = $result.Count % 20
            if ($objectsFinalBatch -eq 0) {
                $objectsFinalBatch = 20
            }
            $objectTracker = 0
            $objectPointer = 0
            $batchCounter = 1
            ...
            do {
                ...
                '
                $bodyUtf8Bytes = [System.Text.Encoding]::UTF8.GetBytes($batches) # Convert to UTF8 bytes
                #Query Graph and sort result
                do {
                    $response = Invoke-RestMethod -Headers $syncHash.headers -Uri "https://graph.microsoft.com/beta/`$batch" -Method Post -Body $bodyUtf8Bytes
                    $responseResponses = $response.responses | Sort-Object {[int]$_.Id}
                    #Check for throttle
                    $isThrottled = $false
                    foreach ($resp in $responseResponses) {
                        if ($resp.body.error.message -like "Too Many Requests*") {
                            Write-Host "Throttled! Sleeping for 10 seconds."
                            Update-AMProgressbar -ProgressbarObject $syncHash.var_progressBar -LabelObject $syncHash.var_lblAction -Action "Searching" -Throttled $true
                ...
                if ($userInput -eq $false -or $serialnumber) {
                    foreach ($responseResponse in $responseResponses) {
                        #Actual datagrid - find by id to handle sorted views
                        $deviceId = $result[$objectPointer].id
                        $datagridItem = $datagridLookup[$deviceId]
                        if ($datagridItem) {
                            $datagridItem.intuneId=$responseResponse.body.id
                            $datagridItem.intuneDeviceName=$responseResponse.body.deviceName
                            $datagridItem.intuneManagedDeviceOwnerType=$responseResponse.body.managedDeviceOwnerType
                            $datagridItem.intuneEnrolledDateTime=$responseResponse.body.enrolledDateTime
                            $datagridItem.intuneOsVersion=$responseResponse.body.osVersion
                            $datagridItem.intuneDeviceEnrollmentType=$responseResponse.body.deviceEnrollmentType
                            $datagridItem.intuneEmailAddress=$responseResponse.body.emailAddress
                            $datagridItem.intuneUserPrincipalName=$responseResponse.body.userPrincipalName
                            $datagridItem.intuneUserDisplayName=$responseResponse.body.userDisplayName
                            $datagridItem.intuneWifiMacAddress=$responseResponse.body.wifiMacAddress
                            $datagridItem.intuneEthernetMacAddress=$responseResponse.body.ethernetMacAddress
                            $datagridItem.intuneFreeStorageSpaceInBytes=$responseResponse.body.freeStorageSpaceInBytes
                            $datagridItem.intuneEnrollmentProfileName=$responseResponse.body.enrollmentProfileName
                        }
                        #Temporary datagrid
                ...
                Update-AMProgressbar -ProgressbarObject $syncHash.var_progressBar -LabelObject $syncHash.var_lblAction -Action "Searching" -Percent $progress
                $batchCounter++
        ...
        if ($autopilotProfile) {
            ...
            do {
                ...
                if ($userInput -eq $false -or $serialnumber) {
                    foreach ($responseResponse in $responseResponses) {
                        #Find by id to handle sorted views
                        $deviceId = $result[$objectPointer].id
                        $datagridItem = $datagridLookup[$deviceId]
                        if ($datagridItem) {
                            $datagridItem.autopilotDeploymentProfile=$responseResponse.body.deploymentProfile.displayName
                        }
    ...
    #region useCache
        ...
        Update-AMCache -Object $syncHash.var_lblCacheSize -Text "Cache size: $($syncHash.cache.Count) devices" -Enabled
        #Enable buttons
        $syncHash.var_btnUpdateGroupTag.IsEnabled = $true
        $syncHash.var_btnDelete.IsEnabled = $true
        $syncHash.var_btnExportSelection.IsEnabled = $true
...
Function Update-AMAutopilotGrouptag {
    [CmdletBinding()]
    Param(
        [Parameter(
            Mandatory=$true,
            HelpMessage='SelectedItems')]
            $Devices
    )
    #Check token status
    if ((Confirm-AMTokenValidity -TokenAcquired $syncHash.tokenAcquired -TokenLifetime $syncHash.token.expires_in) -ne $true) {
    ...
    Update-AMProgressbar -ProgressbarObject $syncHash.var_progressBar -LabelObject $syncHash.var_lblAction -Action "Update" -Percent 0
    $syncHash.var_txtboxGroupTag.Dispatcher.Invoke(
        [action]{
    ...
    #Start updating GroupTag, batches
    $objectsFinalBatch = $devices.Count % 20
    if ($objectsFinalBatch -eq 0) {
        $objectsFinalBatch = 20
    }
    $objectTracker = 0
    $batchCounter = 1
    $countFullBatches = [math]::Ceiling($devices.Count / 20)
    do {
        $batches = '
            {
                "requests": [
            '
        #Determine how many objects to put in batch, limit is always 20, but we might have less objects left
        if (($devices.Count - $objectTracker -lt 20)) {
            #Less than 20 objects remaining
            $x = $objectsFinalBatch
        } else {
            #20 or more objects remaining
                ...
                $batch = @("
                        {
                            `"id`": `"$i`",
                            `"method`": `"POST`",
                            `"url`": `"deviceManagement/windowsAutopilotDeviceIdentities/$($devices[$objectTracker].id)/updateDeviceProperties`",
                            `"body`": {
        ...
        do {
            ...
            foreach ($resp in $responseResponses) {
                if ($resp.body.error.message -like "Too many requests*") {
                    Write-Host "Throttled! Sleeping for 10 seconds."
                    Update-AMProgressbar -ProgressbarObject $syncHash.var_progressBar -LabelObject $syncHash.var_lblAction -Action "Update" -Throttled $true
                    Start-Sleep -Seconds 10
                    $isThrottled = $true
                    break
                }
        ...
        Update-AMProgressbar -ProgressbarObject $syncHash.var_progressBar -LabelObject $syncHash.var_lblAction -Action "Update" -Throttled $false
        ...
        Update-AMProgressbar -ProgressbarObject $syncHash.var_progressBar -LabelObject $syncHash.var_lblAction -Action "Update" -Percent $progress
        $batchCounter++
        #Loop until out of batches
    } while($batchCounter -le $countFullBatches)
    Write-Progress -Activity "Processing Autopilot devices" -Status "Ready" -Completed
    $syncHash.updateGroupTagInProgress = $false
...
Function Remove-AMIntuneDevice {
    param(
    ...
    if ((Confirm-AMTokenValidity -TokenAcquired $syncHash.tokenAcquired -TokenLifetime $syncHash.token.expires_in) -ne $true) {
        ...
        #Disable-AMUI
        [System.Windows.Forms.MessageBox]::Show("Expired or invalid authentication.`nPlease reauthenticate by using `"Login Azure`".","Invalid/expired token","OK","Information") | Out-Null
        $syncHash.deleteIntuneInProgress = $false
        return
    }

    #Reset progress bar
    Update-AMProgressbar -ProgressbarObject $syncHash.var_progressBar -LabelObject $syncHash.var_lblAction -Action "Deleting" -Percent 0
    ...
    do {
        ...
        do {
            ...
            foreach ($resp in $responseResponses) {
                if ($resp.body.error.message -like "Too Many Requests*") {
                    Write-Host "Throttled! Sleeping for 10 seconds."
                    Update-AMProgressbar -ProgressbarObject $syncHash.var_progressBar -LabelObject $syncHash.var_lblAction -Action "Deleting" -Throttled $true
                    Start-Sleep -Seconds 10
                    $isThrottled = $true
                    break
                }
        ...
        Update-AMProgressbar -ProgressbarObject $syncHash.var_progressBar -LabelObject $syncHash.var_lblAction -Action "Deleting" -Throttled $false
        ...
        Update-AMProgressbar -ProgressbarObject $syncHash.var_progressBar -LabelObject $syncHash.var_lblAction -Action "Deleting" -Percent $progress
        $batchCounter++
    } while($batchCounter -le $countFullBatches)
    ...
    if ($statusIntuneDeletion.status -ne 204) {
        ...
        foreach ($status in $statusIntuneDeletion | Where-Object {$_.status -ne 204}) {
...
Function Remove-AMAutopilotDevice {
    param(
    ...
    if ((Confirm-AMTokenValidity -TokenAcquired $syncHash.tokenAcquired -TokenLifetime $syncHash.token.expires_in) -ne $true) {
        ...
        #Disable-AMUI
        [System.Windows.Forms.MessageBox]::Show("Expired or invalid authentication.`nPlease reauthenticate by using `"Login Azure`".","Invalid/expired token","OK","Information") | Out-Null
        $syncHash.deleteAutopilotInProgress = $false
        return
    }

    #Set progress bar
    if ($Intune.IsPresent) {
        Update-AMProgressbar -ProgressbarObject $syncHash.var_progressBar -LabelObject $syncHash.var_lblAction -Action "Deleting" -Percent 50 #Means we just delete some Intune devices
    } else {
        Update-AMProgressbar -ProgressbarObject $syncHash.var_progressBar -LabelObject $syncHash.var_lblAction -Action "Deleting" -Percent 0
    }
    ...
    #Set progress to 100%
    Update-AMProgressbar -ProgressbarObject $syncHash.var_progressBar -LabelObject $syncHash.var_lblAction -Action "Deleting" -Percent 100
    ...
    if ($statusAutopilotDeletion.status -ne 200) {
        ...
        [System.Windows.Forms.MessageBox]::Show("Device(s) successfully deleted.","Autopilot device(s) deleted","OK","Information") | Out-Null
    }
    $syncHash.deleteAutopilotInProgress = $false
    Write-Host "Deletion completed."
}

Function Import-AMAPDevices {
    #Validate csv content with already imported $syncHash.csvDevices
    ...
    #Verify hardware hash in csv. This is not a thorough check
    $missingHwHash = @()
    $line = 0
    foreach ($hash in $syncHash.csvDevices) {
        if ([string]::IsNullOrEmpty($hash.'Hardware Hash')) {
            $missingHwHash += $line
        }
        $line++
    }
    if ([string]::IsNullOrEmpty($missingHwHash) -eq $false) {
        if (($missingHwHash | Measure-Object).Count -gt 10) {
            $missingHashText = "Hardware hash is missing in csv rows: $($missingHwHash -join ", ")."
        } else {
            $missingHashText = "Hardware hash is missing in more than 10 rows."
        }
        [System.Windows.Forms.MessageBox]::Show("$missingHashText","Invalid csv file","OK","Error") | Out-Null
        return
    }
    ...
    do {
        ...
        ")
        $batch = $batch -join " "
        #Import batch to Autopilot
        do {
            $bodyUtf8Bytes = [System.Text.Encoding]::UTF8.GetBytes($batch) # Convert to UTF8 bytes
            try {
                $isThrottled = $false
                $response = Invoke-RestMethod -Headers $syncHash.headers -Uri $uri -Method Post -Body $bodyUtf8Bytes -ErrorAction Stop
                $responses += $response
            } catch {
                if ($_.Exception.Message -like "*status code does not indicate success: 429*") {
                    Write-Host "Throttled! Sleeping for 10 seconds."
                    Update-AMProgressbar -ProgressbarObject $syncHash.var_progressBar -LabelObject $syncHash.var_lblAction -Action "Importing" -Throttled $true
                    Start-Sleep -Seconds 10
                    $isThrottled = $true
                } else {
                    Write-Host "One of the processed batches failed to import. Error: $($_.Exception.Message)"
                    $errorBatches += $batch
                }
            }
        } while ($isThrottled) # Retry if throttled
        Update-AMProgressbar -ProgressbarObject $syncHash.var_progressBar -LabelObject $syncHash.var_lblAction -Action "Importing" -Throttled $false
        #$batches += $batch
        $batchCounter++
    } while($batchCounter -le $countFullBatches)
    #Upload all Autopilot hashes

    #Loop through failed batch responses and create a report (if any)
    if ($errorBatches -ne $null) {
        $errorBatches = $errorBatches | ConvertFrom-Json
        $errorBatchReport = @()
        foreach ($batch in $errorBatches.importedWindowsAutopilotDeviceIdentities) {
            $tempObj = New-Object PSCustomObject -Property ([ordered]@{
                "Device Serial Number"  = $batch.serialNumber
                "Windows Product ID"    = $batch.productKey
                "Hardware Hash"         = $batch.hardwareIdentifier
                "Group Tag"             = $batch.groupTag
                "Assigned User"         = $batch.assignedUserPrincipalName
            })
            $errorBatchReport += $tempObj
        }
        $errorBatchReport = $errorBatchReport | ConvertTo-Csv
        $csv = Write-AMOutfile -Type "AutopilotImportPartialError" -Message $errorBatchReport -FileExtension "csv"
        Open-AMReport -Reportname $csv
        [System.Windows.Forms.MessageBox]::Show("One or more devices in the attached list failed to import.`nPlease review, fix and/or retry the failed devices (usually related to invalid hash or connection issues).`n`nRemaining devices (if any) are still being imported.","Import partially failed","OK","Warning") | Out-Null
    }
    
    #Gather all importIds
    Write-Host "Imported to Autopilot, now waiting for import status..."
    ...
    foreach ($importId in $importIds) {
        ...
        $powershell.AddScript( { Trace-AMAPDeviceImport -ImportId $importId } ) #Code to run
    ...
    $powershell.AddScript( { Watch-AMAPImportRunspace -ImportIds $importIds } ) #Code to run
    $asyncObject = $powershell.BeginInvoke() | Out-Null #Start runspace
}

Function Trace-AMAPDeviceImport {
    ...
    if ($responseStatus.value.state) {
        ...
        $report = Write-AMOutfile -Type "AutopilotImportReport" -Message "Something went wrong. Autopilot upload result was lost. Does not mean upload failed, but we cannot know for certain."
        Open-AMReport -Reportname $report
        return
    }
...
Function Watch-AMAPImportRunspace {
    param(
    [parameter(Mandatory=$true,
        HelpMessage="Provide Autopilot import id to trace status of Autopilot device import.")]
    ...
    foreach ($importId in $importIds) {
        ...
        if ($runspace) {
            ...
            Remove-AMRunspace -id $runspace.id
        }
    }
    #Create and open report
    if ($importMessage) {
        $importMessage = $importMessage | Format-Table -Property * -AutoSize | Out-String -Width 4096
        $report = Write-AMOutfile -Type "AutopilotImportReport" -Message $importMessage
        Open-AMReport -Reportname $report
    }
}

Function Watch-AMRunspace {
    Param(
        [parameter(Mandatory=$true,
            HelpMessage="Provide Custom name of runspace to trace.")]
        [ValidateNotNullorEmpty()][string]$Custom
    )
    do {
        $runspaces = Get-Runspace | Where-Object {$_.Custom -eq $Custom -and $_.RunspaceAvailability -eq "Available"}
        if ($runspaces) {
            Remove-AMRunspace -Custom $Custom
        } else {
            Start-Sleep 10
        }
    } while ($true)
...
Function Write-AMOutfile {
    ...
    param(
        ...
        [parameter(Mandatory=$false,
            HelpMessage="Root folder path of log file.")]
            [ValidateNotNullorEmpty()][string]$FileExtension = "log",
        [parameter(Mandatory=$true,
    ...
    $date = Get-Date -format "yyyy.MM.dd-HH.mm.ss"
    $filename = "$($titleCut)-$($Type)-$($date).$($FileExtension)"
...
Function Compare-AMPSObjectIdentity { # NOT IN USE
    param (
        [Parameter(Mandatory=$true)]
...
Function Invoke-AMMenuItemAction {
...
Function Add-AMMenuItem {
    ...
    foreach ($header in $ItemHeaders) {
        ...
        $menuItem.Add_Click( {
            param($sender)
            Invoke-AMMenuItemAction -Sender $sender -Columns $columns -ContextMenu $ContextMenu
        })
        [void]$contextMenu.Items.Add($menuItem)
    }
}

Function Show-AMCustomMessageBox {
    param (
...
$syncHash.var_btnLoginAzure.Add_Click( {
    ...
    $syncHash.token = Get-AMCodeFlowAuthToken -RedirectUri $redirectUri -Scope $scope -ClientId $syncHash.clientId -WebView2RootDll "$($syncHash.workDirDll)"
    $syncHash.tokenAcquired = Get-Date
    if ($syncHash.token) {
        ...
        $runspace.SessionStateProxy.SetVariable("syncHash",$syncHash) #Add variables
        $runspace.SessionStateProxy.SetVariable("titleCut",$titleCut) #Add variables
        $powershell.AddScript( { Update-AMToken } ) #Code to run
    ...
    #Populate tenantname
    $syncHash.var_lblTenantName.Content = "Tenant name: $(Get-AMTenantName -Token $syncHash.token)"
} )

$syncHash.var_btnLogout.Add_Click( {
    #Logout from Azure and clear token
    Disconnect-AMAzure -ClientId $syncHash.clientId
    $syncHash.var_datagridResults.Items.Clear()
    $syncHash.cache = $null
} )
...
$syncHash.var_btnQuery.Add_Click( {
    ...
    if ($syncHash.csvDevices) {
        ...
        if ($csvDevicesNotFound) {
            $reportName = Write-AMOutfile -Type "CsvDevicesNotFound" -Message $csvDevicesNotFound
            ...
            if ($openReport -eq "Yes") {
                Open-AMReport -Reportname $reportName
            }
        }
    }
    #Count devices visible in datagrid
    $syncHash.var_lblCacheSize.Content -match "(^Cache\ssize:\s\d*\sdevices)" | Out-Null
    Update-AMCache -Object $syncHash.var_lblCacheSize -Text "$($Matches[1]) | Grid count: $($syncHash.var_datagridResults.Items.Count) devices" -Enabled
} )

$syncHash.var_btnUpdateGroupTag.Add_Click( {
    if ($syncHash.var_dataGridResults.SelectedItems.Count -eq 0) {
        [System.Windows.Forms.MessageBox]::Show("Please select at least one device to update group tag.","No selection","OK","Information") | Out-Null
        return
    }
    $continue = Test-AMUpdateLimit
    if ($continue -ne $true) {
        return
    }

    $WatchUpdateGroupTagRunspaceRunspace = Get-Runspace | Where-Object {$_.Custom -eq "WatchUpdateGroupTagRunspace"}
    if (($WatchUpdateGroupTagRunspaceRunspace | Measure-Object).Count -eq 0) {
        $runspace = [runspacefactory]::CreateRunspace($InitialSessionState) #Import functions to runspace
        $runspace.ApartmentState = "STA" #Required for WPF
        $runspace.ThreadOptions = "ReuseThread" #Prevent memory leak
        $powershell = [Powershell]::Create()
        $powershell.Runspace = $runspace
        $runspace.Open()
        $runspace | Add-Member -MemberType NoteProperty -Name "Custom" -Value "WatchUpdateGroupTagRunspace"
        $powershell.AddScript( { Watch-AMRunspace -Custom "UpdateGroupTag" } ) #Code to run
        $asyncObject = $powershell.BeginInvoke() | Out-Null #Start runspace
    }

    if ($syncHash.updateGroupTagInProgress) {
        $choice = [System.Windows.Forms.MessageBox]::Show("Group Tag update is already in progress.`nDo you want to start another update job?","Update in progress","YesNo","Information")
        if ($choice -ne "Yes") {
            return
        }
    }
    $syncHash.updateGroupTagInProgress = $true

    $selectedDevices = @($syncHash.var_dataGridResults.SelectedItems)
    $runspace = [runspacefactory]::CreateRunspace($InitialSessionState) #Import functions to runspace
    $runspace.ApartmentState = "STA" #Required for WPF
    $runspace.ThreadOptions = "ReuseThread" #Prevent memory leak
    $powershell = [Powershell]::Create()
    ...
    $runspace.Open()
    $runspace.SessionStateProxy.SetVariable("syncHash",$syncHash) #Add variables
    $runspace.SessionStateProxy.SetVariable("selectedDevices", $selectedDevices) #Add variables
    $runspace | Add-Member -MemberType NoteProperty -Name "Custom" -Value "UpdateGroupTag"
    $powershell.AddScript( { Update-AMAutopilotGroupTag -Devices $selectedDevices } ) #Code to run
    $asyncObject = $powershell.BeginInvoke() | Out-Null #Start runspace
} )

$syncHash.var_btnExportSelection.Add_Click( {
    if ($syncHash.var_datagridResults.SelectedItems.Count -lt 1) {
        [System.Windows.Forms.MessageBox]::Show("No selections.`nSelect items from grid to export.","No selections","OK","Information") | Out-Null
    } else {
        $date = Get-Date -format "yyyy.MM.dd-HH.mm.ss"
        $filename = "$($titleCut)-Export-$($date).csv"
        try {
            ...
            Start-Sleep 1
            [System.Windows.Forms.MessageBox]::Show("Export made.`nExport location:`n$env:LOCALAPPDATA\$titleCut\$filename`n`nTHIS IS NOT AN AUTOPILOT HARDWARE HASH BACKUP!","Successful export","OK","Information") | Out-Null
        } catch {
            [System.Windows.Forms.MessageBox]::Show("Export failed.`nExport location:`n$env:LOCALAPPDATA\$titleCut\$filename","Failed export","OK","Error") | Out-Null
        }
    }
} )
...
$syncHash.var_btnDelete.Add_Click( {
    $continue = Test-AMDeleteLimit
    $choice = "Cancel"
    if ($continue -eq $false) {
        return
    }

    $WatchDeleteDeviceRunspace = Get-Runspace | Where-Object {$_.Custom -eq "WatchDeleteDeviceRunspace"}
    if (($WatchDeleteDeviceRunspace | Measure-Object).Count -eq 0) {
        $runspace = [runspacefactory]::CreateRunspace($InitialSessionState) #Import functions to runspace
        $runspace.ApartmentState = "STA" #Required for WPF
        $runspace.ThreadOptions = "ReuseThread" #Prevent memory leak
        $powershell = [Powershell]::Create()
        $powershell.Runspace = $runspace
        $runspace.Open()
        $runspace | Add-Member -MemberType NoteProperty -Name "Custom" -Value "WatchDeleteDeviceRunspace"
        $powershell.AddScript( { Watch-AMRunspace -Custom "DeleteDevice" } ) #Code to run
        $asyncObject = $powershell.BeginInvoke() | Out-Null #Start runspace
    }

    if ($syncHash.deleteIntuneInProgress -or $syncHash.deleteAutopilotInProgress) {
        $choice = [System.Windows.Forms.MessageBox]::Show("Device delete job is already in progress.`nDo you want to start another delete job?.","Delete in progress","YesNo","Information")
        if ($choice -ne "Yes") {
            return
        }
    }
    ...
    [array]$selectedDevices = $syncHash.var_dataGridResults.SelectedItems
    $intuneDevices = @()
    ...
    if ($intuneDevices) {
        ...
        $choice = [System.Windows.Forms.MessageBox]::Show("Are you sure you want to delete the selected Autopilot device(s)?","Confirm delete","YesNo","Warning","Button2")
        if ($choice -eq "Yes") {
            $choice = "Autopilot"
        } else {
    ...
    switch ($choice) {
        ...
        "All" {
            ...
            $syncHash.deleteIntuneInProgress = $true
            $syncHash.deleteAutopilotInProgress = $true
            
            $runspaceCode = {
                Remove-AMIntuneDevice -IntuneDevices $intuneDevices -Autopilot
                Remove-AMAutopilotDevice -Intune -Devices $selectedDevices
            }
        }
        "Intune" {
            ...
            $syncHash.deleteIntuneInProgress = $true
            $runspaceCode = {
                Remove-AMIntuneDevice -IntuneDevices $intuneDevices
            }
        }
        "Autopilot" {
            ...
            $syncHash.deleteAutopilotInProgress = $true
            $runspaceCode = {
                Remove-AMAutopilotDevice -Devices $selectedDevices
            }
        }
    }
    ...
    $runspace.Open()
    $runspace.SessionStateProxy.SetVariable("syncHash",$syncHash) #Add variables
    $runspace.SessionStateProxy.SetVariable("intuneDevices", $intuneDevices) #Add variables
    $runspace.SessionStateProxy.SetVariable("selectedDevices", $selectedDevices) #Add variables
    $runspace.SessionStateProxy.SetVariable("titleCut", $titleCut) #Add variables
    $runspace | Add-Member -MemberType NoteProperty -Name "Custom" -Value "DeleteDevice"
    $powershell.AddScript( $runspaceCode ) #Code to run
    $asyncObject = $powershell.BeginInvoke() | Out-Null #Start runspace
} )
...
$syncHash.var_chkboxImportCsv.Add_Click( {
    ...
    if ($syncHash.var_chkboxImportCsv.IsChecked) {
        $syncHash.csvDevices = Import-AMCsvDevices
        if ($syncHash.csvDevices) {
...
#Selection changed event for datagrid, code to avoid too many updates which will slow down the UI.
$syncHash.selectedCount = 0
$syncHash.selectionUpdateTimer = New-Object System.Windows.Threading.DispatcherTimer
$syncHash.selectionUpdateTimer.Interval = [TimeSpan]::FromMilliseconds(200)
$syncHash.selectionUpdateTimer.Add_Tick({
    $syncHash.selectionUpdateTimer.Stop()
    Update-AMGridStatsLabel
})
$syncHash.var_datagridResults.Add_SelectionChanged({
    param($sender, $eventArgs)
    if ($null -eq $syncHash.selectedCount) {
        $syncHash.selectedCount = 0
    }
    $syncHash.selectedCount += $eventArgs.AddedItems.Count
    $syncHash.selectedCount -= $eventArgs.RemovedItems.Count
    if ($syncHash.selectedCount -lt 0) {
        $syncHash.selectedCount = 0
    }
    $syncHash.selectionUpdateTimer.Stop()
    $syncHash.selectionUpdateTimer.Start()
})

#Prepare environment
Start-AMPrepareEnvironment
...
Add-AMMenuItem -ContextMenu $contextMenu -ItemHeader "Select all","Select none","Select default"
#Add columns as menu items
$columns = $syncHash.var_datagridResults.Columns
foreach ($column in $columns) {
    Add-AMMenuItem -ContextMenu $contextMenu -ItemHeader $column.Header -Columns $columns -Visibility
}
...
$syncHash.Window.Add_Closing( {
    param($sender,$eventargs)
    try {
        $disconnectAzure = Disconnect-AMAzure -ClientId $syncHash.clientId
    } catch {
