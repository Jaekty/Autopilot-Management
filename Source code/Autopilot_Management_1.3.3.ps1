...
# 1.3.2 - 03.07.2025 - Updating group tag made into function. Renamed functions to avoid conflict. Rename button backup to export. Bugfix: Support for encoding UTF8 characters; visually and in graph queries. Bugfix: Last batch would not be processed if exactly 20 objects. 
...
Function Disable-AMUI {
    ...
    Update-AMProgressbar -Object $syncHash.var_progressBar -Percent 0
    ...
    if ($useCache -eq $false) {
        ...
        if ($intune) {
            ...
            do {
                ...
                Update-AMProgressbar -Object $syncHash.var_progressBar -Percent $progress
                $batchCounter++
...
Function Remove-AMAutopilotDevice {
    ...
    #Set progress bar
    if ($Intune.IsPresent) {
        Update-AMProgressbar -Object $syncHash.var_progressBar -Percent 50 #Means we just delete some Intune devices
    } else {
        Update-AMProgressbar -Object $syncHash.var_progressBar -Percent 0
    }
    ...
    #Set progress to 100%
    Update-AMProgressbar -Object $syncHash.var_progressBar -Percent 100
...
Function Import-AMAPDevices {
    ...
    foreach ($query in $batches) {
        do {
            $bodyUtf8Bytes = [System.Text.Encoding]::UTF8.GetBytes($query) # Convert to UTF8 bytes
            try {
                $isThrottled = $false
                $response = Invoke-RestMethod -Headers $syncHash.headers -Uri $uri -Method Post -Body $bodyUtf8Bytes -ErrorAction Stop
                $responses += $response
                ...
                if ($_.Exception.Message -like "*status code does not indicate success: 429*") {
                    ...
                    Write-Host "Something went wrong. One or more devices failed to import. Error: $($_.Exception.Message)"
                    $errorBatch += $response
                    [System.Windows.Forms.MessageBox]::Show("Something went wrong.`nError: $($_.Exception.Message)","Import failed","OK","Error") | Out-Null
                }
            }
        } while ($isThrottled) # Retry if throttled
    }
...
$syncHash.var_btnDelete.Add_Click( {
    ...
    switch ($global:choice) {
        ...
        "All" {
            ...
            Remove-AMAutopilotDevice -Intune
        }
        "Intune" {
